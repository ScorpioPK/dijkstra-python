...
#I fixed a small issue when the heap contained equal keys. See comment on line 135
import math
        ...
        for w in range(0, len(nodes)):
            ...
            while A[d] != None:
                ...
                if x.key > y.key:
                    ...
                    x, y = y, temp
                self.heap_link(y, x)
                #since y becomes a child of x, it means that y.key >= x.key, therefore x is a better min_node
                #and this also fixes a potential issue when there are equal keys in the tree:
                # y becomes min_node and it is then set as a child of x, therefore being removed from the root_list
                if self.min_node.value == y.value:
                    self.min_node = x
                A[d] = None
                d += 1
            A[d] = x
